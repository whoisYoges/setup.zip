...
if [[ $username == root ]] ; then
	...
	echo "STAY CLOSE TO KEYBOARD. YOU NEED TO CONFIRM THE INSTALLATION AND ENTER YOUR PASSWORD SEVERAL TIMES DURING THE PROCESS."
	sleep 2s
	echo "Installing Git..."
...
# 	doas ufw allow cifs
	...
	cd ~/Gitclones/anime-terminal || return
	echo "Moving anime-terminal to local bin"
	chmod +x anime-terminal
	doas mv anime-terminal /usr/local/bin/
	cd ~/Gitclones/ytfzf || return
	chmod +x ytfzf 
	echo "Moving ytfzf to local bin"
	doas mv ytfzf /usr/local/bin/
	cd ~/ || exit
	rm -Rf Gitclones

	cd ~/Applications/suckless/dwm-6.2/ || exit
